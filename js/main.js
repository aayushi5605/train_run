/**
 * main.js - Game Entry Point (Legacy)
 * Main game initialization and render loop - Note: Replaced by game-engine.js
 * Contains global game state variables and core game logic
 */

let statusKeys = {};
let cubeRotation = 0.0;
let cameraAngleDegHoriz = 0;
let cameraAngleDegVert = 0;
const num_walls = 30;
let flash = false;
let gray = false;
let game_over = false;
...
let speed = 0.0336;
let speed_wall = 0.045;
const base_speed = 0.0336;
const base_speed_wall = 0.045;
...
function main() {
  ...
  void main(void) {
    ...
    highp vec3 ambientLight = vec3(0.5, 0.5, 0.6);

    highp vec3 directionalLightColor = vec3(0.7, 0.7, 0.8);
    if (flash) {
      directionalLightColor = vec3(1.8, 1.8, 2.0);
    }
    highp vec3 directionalVector = normalize(vec3(0.2, 0.8, 0.3));
  ...
  const finish_object = finishline(gl);
  const buffer_finish_object = initBuffers(gl, finish_object);
  ...
  // Draw the scene repeatedly - optimized
  let frameCount = 0;
  function render(now) {
    ...
    // Performance optimization: limit some calculations
    frameCount++;
    ...
    // Update score less frequently for better performance
    if (frameCount % 2 === 0) {
      update_score();
    }
    
    // Progressive speed increase based on distance
    if (!game_over && !finish && game_start && objects[0]) {
      const distance = objects[0].distance || 0;
      speed = base_speed + (distance * speed_increase_rate);
      speed_wall = base_speed_wall + (distance * speed_increase_rate * 1.9);
      
      // Cap maximum speed to avoid game becoming unplayable
      speed = Math.min(speed, base_speed * 2.5);
      speed_wall = Math.min(speed_wall, base_speed_wall * 2.5);
    }
    
    if (game_over || finish) {
      // Pass coins and distance to Game_over
      coinsCollected = objects[0].coins;
      distanceTraveled = objects[0].distance;
      Game_over();
    }
    const projectionMatrix = clearScene(gl, objects[0].translate);

    const rand = getRandomInt(1, 180); // Reduced from 200 for more frequent spawns
    if (!game_over && !finish && game_start) {
      if (rand % 10 == 0) { // Changed from 13 to 10 for faster spawning
